...
export const GET: RequestHandler = async ({ url, locals }) => {
	...
	if (code) {
		const { data: sessionData, error } = await locals.supabase.auth.exchangeCodeForSession(code);
		...
		if (!error && sessionData?.user) {
			...
			// (via invite link OR direct sign-in - this is a single-trip app)
			...
			if (!existingMember && isInvite) {
				// First member becomes admin, everyone else is editor
